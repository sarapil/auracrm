"""
AuraCRM — Custom Exception Hierarchy
...
class AuraCRMError(Exception):
    """AuraCRM base exception — all app-specific errors inherit from this."""

    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in AuraCRM")
        self.title = title or frappe._("AuraCRM Error")
        super().__init__(self.message)


class ValidationError(AuraCRMError):
...
class NotFoundError(AuraCRMError):
...
class PermissionError(AuraCRMError):
...
class ConfigurationError(AuraCRMError):
...
class IntegrationError(AuraCRMError):
...
class RateLimitError(AuraCRMError):

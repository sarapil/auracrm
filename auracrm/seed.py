...
def seed_data():
    ...
    """
    frappe.logger().info("AuraCRM: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("AuraCRM: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["AC Settings", "AuraCRM Settings", "Auracrm Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "AC User", "desk_access": 1},
        {"role": "AC Manager", "desk_access": 1},
        {"role": "AC Admin", "desk_access": 1},
    ]

# Contributing to AuraCRM — المساهمة في AuraCRM
...
```bash
git clone https://github.com/YOUR_USERNAME/auracrm.git
cd auracrm
```
...
```bash
bench get-app auracrm /path/to/your/fork
...
bench --site dev.localhost run-tests --app auracrm --module auracrm.tests.unit.test_my_service
```
...
- Update help files in `auracrm/help/` for DocType changes
